import org.junit.jupiter.api.Test;
import org.junit.jupiter.api.TestInstance;

import static org.junit.jupiter.api.Assertions.assertEquals;

@TestInstance(TestInstance.Lifecycle.PER_CLASS)
public class repeated_word_test {
    app classSample=new app();
@Test
    public void TestFirstSentence() {
    String sentence="Once upon a time, there was a brave princess who...";
    String actual=classSample.repeated_word(sentence);
    String excepted="a";
    assertEquals(excepted, actual);
}
    @Test
    public void TestSecondSentence() {
        String sentence="It was the best of times, it was the worst of times," +
                " it was the age of wisdom, it was the age of foolishness," +
                " it was the epoch of belief, it was the epoch of incredulity," +
                " it was the season of Light, it was the season of Darkness," +
                " it was the spring of hope, it was the winter of despair," +
                " we had everything before us, we had nothing before us," +
                " we were all going direct to Heaven, " +
                "we were all going direct the other way – in short," +
                " the period was so far like the present period," +
                " that some of its noisiest authorities insisted on its being received," +
                " for good or for evil, in the superlative degree of comparison only...";
        String actual=classSample.repeated_word(sentence);
        String excepted="it";
        assertEquals(excepted, actual);
    }
    @Test
    public void TestThirdSentence() {
        String sentence="It was a queer, sultry summer," +
                " the summer they electrocuted the Rosenbergs," +
                " and I didn’t know what I was doing in New York...";
        String actual=classSample.repeated_word(sentence);
        String excepted="summer";
        assertEquals(excepted, actual);
    }

}
